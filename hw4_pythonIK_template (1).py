...
def jacobian_transposed(robot, q0, Rd, Pd, Nmax, alpha, tol):
    ...
    while (np.absolute(dX) > tol).any():
        ...
        if iternum < Nmax:
            ...
            # compute the update
            j = np.matmul(np.transpose(Jq), dX)
            # use the update to generate a new q
            q[:, iternum+1] = q[:, iternum] - alpha * np.transpose(j)
            iternum = iternum + 1
        else:
            break
...
def main():
    # define inputs for jacobian_transposed function
    ...
    # compute the inverse kinematics using Jacobian transposed
    q = jacobian_transposed(robot, q0, Rd, Pd, Nmax, alpha, tol)
